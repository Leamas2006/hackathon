    ...
    def __init__(self, base_dir: Union[str, Path]):
        ...
        self.s3 = boto3.client(
            ...
            config=boto3.session.Config(
                ...
                s3={"addressing_style": "path"},  # Use path-style S3 URLs
            ),
        )
        ...
        if not base_dir_str.startswith("s3://"):
            # Try to fix common conversions
            if base_dir_str.startswith("s3:/"):
                base_dir_str = "s3://" + base_dir_str[4:]
            elif base_dir_str.startswith("s3:\\"):
                base_dir_str = "s3://" + base_dir_str[5:]
            else:
                raise ValueError("base_dir must be an S3 URI (s3://bucket/path)")

        # Parse S3 URI - use base_dir_str not base_dir
        parts = base_dir_str[5:].split("/", 1)
        self.bucket = parts[0]
